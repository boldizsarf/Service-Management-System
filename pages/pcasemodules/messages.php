<div class="col-md-12">
    ...
    <div class="card card-primary" <?php if ($hasmsg == false) { echo "hidden"; }?>>
        ...
        <div class="card-body">
            <?php
            $dbmessages = $dbc->get("SELECT * FROM pmessages WHERE cid=?", [$case["CaseID"]]);
            foreach ($dbmessages as $msg) {
                $dbmsguser = $dbc->get("SELECT * FROM users WHERE id=?", [$msg["uid"]]);
                if ($case["Partner"]["ID"] == $msg["uid"]) {
                    $msgusertext = $case["Partner"]["Store"];
                } else {
                    $msgusertext = $dbmsguser[0]["lastname"] . " " . $dbmsguser[0]["firstname"];
                }
                $msguser = $dbmsguser[0];
                ...
                echo "                                                                <div class=\"font-weight-600\">" . $msgusertext . "</div>\n";
...
<div class="col-md-12">
    <div class="card card-primary">
        ...
        <div class="card-body">
            <form method="POST" action="/backend/pcaseman.php?q=sendmsg">
